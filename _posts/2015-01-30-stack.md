...
A stack window displays a 3D image stack, one image plane (slice) at a time.
 ...
 - Use the mouse-wheel to **scroll** up and down through images.
 - Click and drag will **pan** the image. Arrow keys will also pan.
 - Ctrl+mouse wheel to **zoom** in and out. Keyboard +/- will also zoom in and out. All zooming is done by following the mouse pointer.
 - Image **contrast** is critical, open the contrast control bar with keyboard 'c' and adjust min/max contrast with sliders. If contrast is not set properly you may not see dim but potentially important structures in your images.
...
Please have a look through all of these image visualization features. Try them all out on an actual stack, they are designed to make workflows more efficient. Finally, please don't forget [stack annotations](#stack-annotations).
...
The contrast bar is used to set image contrast in a stack window.
...
Use keyboard '1' and '2' to switch between channels one and two. Channels can also be selected using the right-click menu.
...
Make new 3D points with shift+click. **All points are 3D**, the z position of a point will be the current viewed image when the 3D point is created. See [annotating a stack][3] for details on creating and editing 3D points and lines. To easily manage a stack db, open the left stack db panel with keyboard '[' or use the [point list][4] panel.
