...
The x/y/z voxel size of each stack is set with keyboard 'shift+p'. The voxel size must be set before and annotations are created. Once annotations are created, you should not change the voxel size. Make sure the voxel size is correct.

#### Scale bar
Each stack will display a white scale bar in um. Control the length of the scale bar in [Options][2]. 

<div class="print-page-break"></div>

#### Viewing multiple channels

Use keyboard '1' and '2' to switch between channels.
